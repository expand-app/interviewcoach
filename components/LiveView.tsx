...
function CommentarySection({
  ...
  // the hint take the pane. Outside the eligible states (or without a
  ...
  // States in which a listening hint is meaningful to display:
  //   - interviewer_speaking: classic setup monologue ("Tell me about a
  //     time you balanced X with Y…")
  //   - question_finalized: the question has been locked but the
  //     interviewer is still expanding on it ("…banking is regulatory-
  //     heavy, interpretability is key, documentation is 2500 pages…").
  //     Without this state included the hint would be generated by the
  //     orchestrator but invisible — exactly the bug we saw in the 32-min
  //     transcript where useful coaching never reached the user.
  // Excluded: idle / chitchat (no real setup), candidate_questioning
  // (we're evaluating the candidate's question, not coaching listening).
  const HINT_VISIBLE_STATES: MomentStateKind[] = [
